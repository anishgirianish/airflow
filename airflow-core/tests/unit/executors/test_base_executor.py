...
class TestCallbackSupport:
    def test_supports_callbacks_flag_default_false(self):
        executor = BaseExecutor()
        assert executor.supported_workload_types == frozenset({WorkloadType.EXECUTE_TASK})
    ...
    def test_local_executor_supports_callbacks_true(self):
        """Test that LocalExecutor supports both EXECUTE_TASK and EXECUTE_CALLBACK."""
    ...
    def test_queue_callback_without_support_raises_error(self, dag_maker, session):
        executor = BaseExecutor()  # WorkloadType.EXECUTE_CALLBACK not in supported_workload_types by default
