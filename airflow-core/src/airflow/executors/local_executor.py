...
def _run_worker(
    ...
    while True:
        ...
        # Connection tests handle their own result reporting via the Execution API,
        # so they are not put on the result queue.
...
def _execute_workload(log: Logger, workload: ExecutorWorkload, team_conf) -> None:
    ...
    """
    setproctitle(f"{_get_executor_process_title_prefix(team_conf.team_name)} {workload.display_name}", log)
...
class LocalExecutor(BaseExecutor):
    ...
    def _process_workloads(self, workload_list):
        for workload in workload_list:
            ...
            if isinstance(workload, workloads.TestConnection):
                self.queued_connection_tests.pop(str(workload.connection_test_id), None)
            else:
