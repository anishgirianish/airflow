...
from airflow.sdk.execution_time import supervisor as _sdk_supervisor
...
def _route_lookups_to_test_request(conn_id: str, conn_response: ConnectionResponse):
    """Prepend a secrets backend that resolves ``conn_id`` to the test request fields."""
    original_loader = _sdk_supervisor.ensure_secrets_backend_loaded
    backend = _TestRequestSecretsBackend(conn_id, conn_response)

    def loader() -> list[BaseSecretsBackend]:
        return [backend, *original_loader()]

    _sdk_supervisor.ensure_secrets_backend_loaded = loader
    try:
        yield
    finally:
        _sdk_supervisor.ensure_secrets_backend_loaded = original_loader
...
def supervise_connection_test(
    *,
    ...
    try:
        ...
        with _route_lookups_to_test_request(conn_response.conn_id, conn_response):
